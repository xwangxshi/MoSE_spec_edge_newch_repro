...
@register.register_scheduler('StepLR_with_warmup')
def StepLR_warmup_scheduler(optimizer: Optimizer,
                                 num_warmup_epochs: int, max_epoch: int):
    scheduler = get_StepLR_with_warmup(
        optimizer=optimizer,
        num_warmup_steps=num_warmup_epochs,
        num_training_steps=max_epoch
    )
...
@register.register_scheduler('custom_cosine_with_warmup')
def custom_cosine_with_warmup_scheduler(optimizer: Optimizer,
                                 num_warmup_epochs: int, max_epoch: int):
    scheduler = get_cosine_schedule_with_warmup(
        optimizer=optimizer,
        num_warmup_steps=num_warmup_epochs,
...
def get_custom_cosine_schedule_with_warmup(
    ...
    """
    Copies the implementation of Cosine annealing with warmup from HomBasis codebase
    """

    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, num_training_steps, eta_min=0.00005, last_epoch=last_epoch)
    warmup_scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer, start_factor=0.05, end_factor=1, total_iters=num_warmup_steps
    )
...
def get_StepLR_with_warmup(
        optimizer: Optimizer, num_warmup_steps: int, num_training_steps: int, last_epoch: int = -1):

    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1, last_epoch=last_epoch)
    warmup_scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer, start_factor=0.05, end_factor=1, total_iters=num_warmup_steps
    )
    scheduler = torch.optim.lr_scheduler.SequentialLR(
