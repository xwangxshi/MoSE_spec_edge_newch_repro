...
class KernelPENodeEncoder(torch.nn.Module):
    ...
    def __init__(self, dim_emb, expand_x=True):
        ...
        if model_type == 'mlp':
            ...
            self.pe_encoder = MLP(in_channels=num_rw_steps, hidden_channels=dim_pe, out_channels=dim_pe, num_layers=n_layers, norm=pecfg.norm)
        elif model_type == 'mlp':
            if hasattr(cfg.posenc_RWSE, "hid_channel_list"):
                channel_list = cfg.posenc_RWSE.hid_channel_list
                channel_list.append(dim_pe)
                channel_list.insert(0,num_rw_steps)
            else:
                raise Exception('Must provide cfg.posenc_RWSE.hid_channel_list')
            self.pe_encoder = MLP(channel_list=channel_list, num_layers=n_layers, norm=pecfg.norm)
        else:
