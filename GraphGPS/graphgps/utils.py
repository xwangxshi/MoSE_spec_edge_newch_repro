...
# For follow batch edge_attr functionalitiy:
...
def custom_get_loader(dataset, sampler, batch_size, shuffle=True):
    ...
    if sampler == "full_batch" or len(dataset) > 1:
        loader_train = DataLoader(dataset, batch_size=batch_size,
                                  shuffle=shuffle, follow_batch=["edge_attr"], num_workers=cfg.num_workers,
...
def custom_create_loader():
    ...
    for i in range(cfg.share.num_splits - 1):
        if cfg.dataset.task == 'graph':
            ...
            loaders.append(
                custom_get_loader(dataset[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
            delattr(dataset.data, split_names[i])
        else:
            loaders.append(
                custom_get_loader(dataset, cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
