...
if [[ "$#" -ne 10 ]]; then
  echo "Usage: $0 TAG CAMPAIGN CONFIG PHYSICAL_GPU ZERO_VN HIDDEN_DP OUTPUT_DP FILTER_DP SPEC_WD EXPECTED_PARAMS" >&2
  exit 64
fi
...
case "$CAMPAIGN" in
  "$GRAPHGPS"/results_specmose_newch_stage2_*) ;;
...
(
  ...
  PYTHONPATH=. PYTHONDONTWRITEBYTECODE=1 "$PY" \
    run/verify_specmose_model_params.py \
    --cfg "$CFG" \
